...
if [[ "$JAVA_VER" != "250" ]]; then
  echo test-lambda works only with Java 25
  exit 1;
fi
...
./gradlew micronaut-cli:run --args="create-app --jdk 25 -b gradle -f \"aws-lambda,aws-cdk,amazon-api-gateway-http\" temp1" || EXIT_STATUS=$?
...
./gradlew micronaut-cli:run --args="create-function-app --jdk 25 -b gradle -f \"aws-lambda,aws-cdk,aws-lambda-function-url\" temp2" || EXIT_STATUS=$?
...
./gradlew micronaut-cli:run --args="create-app --jdk 25 -b gradle -f \"aws-lambda,aws-cdk,amazon-api-gateway\" temp3" || EXIT_STATUS=$?
...
./gradlew micronaut-cli:run --args="create-function-app --jdk 25 -b gradle -f \"aws-lambda,aws-cdk,amazon-api-gateway\" temp4" || EXIT_STATUS=$?
