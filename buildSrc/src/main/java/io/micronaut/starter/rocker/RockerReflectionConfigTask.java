    ...
    public void assemble() throws IOException {
        ...
        getSource().visit(fileVisitDetails -> {
            ...
            if (relativePath.getLastName().endsWith(suffix)) {
                ...
                Map<String, Object> plainText = new HashMap<>(2);
                plainText.put("name", className + "$PlainText");
                plainText.put("allDeclaredConstructors", true);
                plainText.put("allDeclaredFields", true);
                json.add(plainText);
            }
        });
