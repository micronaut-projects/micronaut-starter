...
class HibernateGormSpec extends BeanContextSpec {
    ...
    void "test dependencies are present for gradle"() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["hibernate-gorm"]), new GradleBuild()).render().toString()
    ...
    void "test dependencies are present for maven"() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["hibernate-gorm"]), new MavenBuild()).render().toString()
