...
class TelegramAzureChatBotSpec extends BaseTelegramChatBotSpec {
    ...
    void 'test README contains docs for #buildTool and command "#command"'(BuildTool buildTool, String command) {
        when:
        def output = generate(ApplicationType.FUNCTION, new Options(Language.JAVA, buildTool), [TelegramAzureChatBot.NAME])
        def readme = output["README.md"]
        def expectedCmd = beanContext.getBean(TelegramAzureChatBot).getBuildCommand(buildTool)

        then:
        readme.contains("Telegram ChatBot")
        readme.contains("./$expectedCmd")
