...
class VelocitySpec extends BeanContextSpec  implements CommandOutputFixture {
    ...
    void 'test gradle views-velocity feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['views-velocity'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven views-velocity feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['views-velocity'], language), new MavenBuild()).render().toString()
