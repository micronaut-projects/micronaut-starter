...
class FreemarkerSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void 'test gradle views-freemarker feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['views-freemarker'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven views-freemarker feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['views-freemarker'], language), new MavenBuild()).render().toString()
