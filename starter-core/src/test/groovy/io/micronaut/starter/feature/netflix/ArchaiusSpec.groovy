...
class ArchaiusSpec extends BeanContextSpec {
    ...
    void 'test gradle netflix-archaius feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['netflix-archaius'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven netflix-archaius feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['netflix-archaius'], language), new MavenBuild()).render().toString()
