...
class SQLServerSpec extends BeanContextSpec {
    ...
    void 'test gradle sqlserver feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['sqlserver'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven sqlserver feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['sqlserver'], language), new MavenBuild()).render().toString()
