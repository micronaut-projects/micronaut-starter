...
class AcmeSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void 'test gradle acme feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['acme'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven acme feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['acme'], language), new MavenBuild()).render().toString()
