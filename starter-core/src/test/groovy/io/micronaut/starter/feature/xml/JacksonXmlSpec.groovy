...
class JacksonXmlSpec extends BeanContextSpec  implements CommandOutputFixture {
    ...
    void 'test gradle jackson-xml feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['jackson-xml'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven jackson-xml feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['jackson-xml'], language), new MavenBuild()).render().toString()
