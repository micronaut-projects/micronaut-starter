...
class RibbonSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void 'test gradle netflix-ribbon feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['netflix-ribbon'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven netflix-ribbon feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['netflix-ribbon'], language), new MavenBuild()).render().toString()
