...
class SpringDataJpaSpec extends BeanContextSpec {
    ...
    void 'test spring-data-jpa with Gradle for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['spring-data-jpa'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven spring-data-jpa feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['spring-data-jpa'], language), new MavenBuild()).render().toString()
