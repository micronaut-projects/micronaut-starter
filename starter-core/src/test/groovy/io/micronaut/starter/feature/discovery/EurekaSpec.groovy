...
class EurekaSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void 'test gradle discovery-eureka feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['discovery-eureka'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven discovery-eureka feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['discovery-eureka'], language), new MavenBuild()).render().toString()
