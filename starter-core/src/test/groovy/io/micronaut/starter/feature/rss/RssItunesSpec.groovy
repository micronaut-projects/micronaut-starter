...
class RssItunesSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void 'test gradle rss-itunes-podcast feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['rss-itunes-podcast'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven rss-itunes-podcast feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['rss-itunes-podcast'], language), new MavenBuild()).render().toString()
