...
class RssSpec extends BeanContextSpec  implements CommandOutputFixture {
    ...
    void 'test gradle rss feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['rss'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven rss feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['rss'], language), new MavenBuild()).render().toString()
