...
class MySQLSpec extends BeanContextSpec {
    ...
    void 'test gradle mysql feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['mysql'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven mysql feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['mysql'], language), new MavenBuild()).render().toString()
