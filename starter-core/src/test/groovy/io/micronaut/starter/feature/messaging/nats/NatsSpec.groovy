...
class NatsSpec extends BeanContextSpec {
    ...
    void "test dependencies are present for gradle"() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["nats"]), new GradleBuild()).render().toString()
    ...
    void "test dependencies are present for maven"() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["nats"]), new MavenBuild()).render().toString()
