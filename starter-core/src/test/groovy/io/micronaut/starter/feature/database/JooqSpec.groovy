...
class JooqSpec extends BeanContextSpec  implements CommandOutputFixture {
    ...
    void 'test gradle jooq feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['jooq'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven jooq feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['jooq'], language), new MavenBuild()).render().toString()
