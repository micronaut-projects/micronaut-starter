...
class Neo4jGormSpec extends BeanContextSpec {
    ...
    void "test dependencies are present for gradle"() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["neo4j-gorm"]), new GradleBuild()).render().toString()
    ...
    void "test dependencies are present for maven"() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["neo4j-gorm"]), new MavenBuild()).render().toString()
