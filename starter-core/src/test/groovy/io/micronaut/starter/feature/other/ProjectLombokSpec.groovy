...
class ProjectLombokSpec extends BeanContextSpec  implements CommandOutputFixture {
    ...
    void 'test lombok with Gradle for Java'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['lombok'], Language.JAVA)).render().toString()

        then:
        template.contains('annotationProcessor("org.projectlombok:lombok:${lombokVersion}")')
        template.contains('compileOnly("org.projectlombok:lombok:${lombokVersion}")')
    }
    ...
    void 'test lombok with Gradle for only Java'() {
        ...
        buildGradle.template(ApplicationType.DEFAULT, buildProject(),
                getFeatures(['lombok'], language)).render().toString()
