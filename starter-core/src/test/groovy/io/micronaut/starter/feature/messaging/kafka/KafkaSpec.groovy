...
class KafkaSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void "test dependencies are present for gradle"() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["kafka"]), new GradleBuild()).render().toString()
    ...
    void "test dependencies are present for maven"() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(["kafka"]), new MavenBuild()).render().toString()
