...
class HazelcastSpec extends BeanContextSpec implements CommandOutputFixture {
    ...
    void 'test gradle cache-hazelcast feature for language=#language'() {
        when:
        String template = buildGradle.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['cache-hazelcast'], language), new GradleBuild()).render().toString()
    ...
    void 'test maven cache-hazelcast feature for language=#language'() {
        when:
        String template = pom.template(ApplicationType.DEFAULT, buildProject(), getFeatures(['cache-hazelcast'], language), new MavenBuild()).render().toString()
